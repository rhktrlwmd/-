...
В этой лабораторной работе рассматривается базовая модель экспоненциального роста. В литературном источнике объединены краткое теоретическое описание и исполняемый код, из которого автоматически извлекаются чистый `Julia`-скрипт и `ipynb`.
...
Полученные таблицы содержат основную траекторию и параметрический анализ по коэффициенту роста. Далее эти таблицы используются для построения графиков и формирования отчёта.
